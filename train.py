...
if __name__ == "__main__":
    # --------------- Interactive setup ---------------
    print("=== SO-101 Training Setup ===\n")
    HF_USER = input("HuggingFace username: ").strip()
    DATASET_NAME = input("Dataset name (on HF Hub): ").strip()
    REPO_ID = f"{HF_USER}/{DATASET_NAME}"
    print(f"\nTraining on dataset: {REPO_ID}\n")

    # --------------- Constants ---------------
    POLICY_TYPE = "act"
    OUTPUT_DIR = f"outputs/train/{POLICY_TYPE}_{DATASET_NAME}"
    JOB_NAME = f"{POLICY_TYPE}_{DATASET_NAME}"

    DEVICE = "mps"              # "cuda" for NVIDIA, "mps" for Apple Silicon, "cpu" for CPU
    WANDB_ENABLED = False
    PUSH_TO_HUB = True
    POLICY_REPO_ID = f"{HF_USER}/{POLICY_TYPE}_{DATASET_NAME}"
    # -----------------------------------------
